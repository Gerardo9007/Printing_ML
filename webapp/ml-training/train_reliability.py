"""Train the Phase-1 reliability classifier and export plain-JSON artifacts.

Fits StandardScaler + LogisticRegression on ml-training/data/reliability_dataset.csv,
then writes the runtime artifacts the backend loads with pure numpy:
  webapp/backend/ml/reliability_model.json   (coef, intercept, scaler_mean, scaler_scale)
  webapp/backend/ml/reliability_meta.json    (model_type, schema_version, feature_names, ...)

The feature order comes from the SAME reliability.FEATURE_NAMES the generator and the
runtime use, so training and inference can never drift (ML-ARCHITECTURE.md §4/§5).
"""
...
from reliability import FEATURE_NAMES, FEATURE_SCHEMA_VERSION  # noqa: E402

DATA_DIR = os.path.join(os.path.dirname(__file__), "data")
CSV_PATH = os.path.join(DATA_DIR, "reliability_dataset.csv")
ML_DIR = os.path.join(_BACKEND_DIR, "ml")
MODEL_PATH = os.path.join(ML_DIR, "reliability_model.json")
META_PATH = os.path.join(ML_DIR, "reliability_meta.json")
...
def main():
    ...
    print("=== trained reliability classifier ===")
