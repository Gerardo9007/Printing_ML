...
import type { DetectionItem, ImageUrls } from "@/lib/api";

export type ImageTab = "detections" | "reference" | "aligned" | "diff_mask";

const TABS: { key: ImageTab; label: string }[] = [
  { key: "detections", label: "Detections" },
  { key: "reference", label: "Reference" },
  { key: "aligned", label: "Aligned" },
  { key: "diff_mask", label: "Diff mask" },
];

// Overlay only makes sense on tabs sharing aligned.png's coordinate space.
const OVERLAY_TABS: ImageTab[] = ["detections", "aligned"];
...
export default function AnnotatedImage({
  ...
  activeTab: ImageTab;
  onTabChange: (t: ImageTab) => void;
  selectedIndex: number | null;
  hoverIndex: number | null;
}) {
  const imgRef = useRef<HTMLImageElement>(null);
  // Natural dimensions of the loaded image, needed to scale bbox overlays.
    ...
    <section className="space-y-3">
      ...
      <div className="relative overflow-hidden rounded-lg border border-[var(--border-hairline)] bg-surface">
        ...
        {showOverlay && target && natural && (
          ...
          >
            {target.oversized && (
              <span className="absolute left-0 top-0 -translate-y-full whitespace-nowrap rounded-t bg-status-warning px-1 text-[10px] font-medium text-black">
                ⚠ oversized
              </span>
            )}
          </div>
