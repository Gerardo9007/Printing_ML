...
import {
  getDieBladeResult,
  getDieBladeHistory,
  readStashedDieBladeResult,
  friendlyMessage,
  ApiError,
  type DieBladeAnalyzeResponse,
  type DieBladeHistoryItem,
} from "@/lib/api";
import MetricsPanelDieBlade from "@/components/dieblade/MetricsPanelDieBlade";
import AnnotatedImageDieBlade, {
  type DieBladeImageTab,
} from "@/components/dieblade/AnnotatedImageDieBlade";
import DetectionListDieBlade from "@/components/dieblade/DetectionListDieBlade";

type LoadState =
  | { status: "loading" }
  | { status: "ok"; data: DieBladeAnalyzeResponse }
  | { status: "notfound" }
  | { status: "error"; message: string };

export default function ResultViewDieBlade({ id }: { id: string }) {
  const [state, setState] = useState<LoadState>({ status: "loading" });
  const [reloadKey, setReloadKey] = useState(0);

  const [activeTab, setActiveTab] = useState<DieBladeImageTab>("detections");
  const [selectedIndex, setSelectedIndex] = useState<number | null>(null);
  const [hoverIndex, setHoverIndex] = useState<number | null>(null);

  const [history, setHistory] = useState<DieBladeHistoryItem[]>([]);
  ...
  useEffect(() => {
    const stashed = readStashedDieBladeResult(id);
    if (stashed) {
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-8">
      ...
      <div className="space-y-6">
        <MetricsPanelDieBlade
          registration={data.registration}
          metrics={data.metrics}
        />

        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <AnnotatedImageDieBlade
