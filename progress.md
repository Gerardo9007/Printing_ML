...
### ⑫ 2026-07-12 — task1 데이터셋 확대 재학습 (recall 유지 여부 확인)
- 사용자 요청으로 misalign 변주 6→20회로 늘려 데이터셋 재생성: 60쌍/240실행/1,242행 → **300쌍/1,200실행/5,601행**(positive 1202/negative 4399)
- 재학습 결과: accuracy 97.4%, precision 89.6%, **recall 99.7%**(FN 1/301) — 이전(100%)보다 문자 그대로는 소폭 하락
- 유일한 FN 진단: k=2.0(불안정 회귀 테스트 구간, 오검출 가드레일 20%에 근접하는 면적비 14%)에서 나온 경계 케이스로, 실사용 회귀 게이트와는 무관한 통계적 노이즈로 판단
- **실사용 회귀 3종 재확인**: 5결함 통합 데모(recall 100%, critical_missed=False), 성분표시오류 단독(critical_missed=False), 진짜 무결함(참조=참조 자기비교, 검출 0건) — 전부 이상 없음
- 부수 발견: 그동안 "오검출 케이스"로 불렀던 `webapp/test-images/reference.png` 자체가 실은 `docs/dev` 기본 참조본과 완전히 동일하지 않음(폰트 렌더링 미세 차이로 실제 diff 존재) — 진짜 무결함 검증은 참조 이미지를 자기 자신과 비교하는 방식으로 대체해야 정확함
- **결정**: 사용자가 5,601행 버전 유지 확정 → 커밋
